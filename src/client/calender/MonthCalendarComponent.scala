...
class MonthCalendarComponent extends StackPane {
  ...
  val headerFont: Font =Font.font("Arial",12d)
  val weekFont: Font =Font.font("Arial",12d)
  val daysFont: Font =Font.font("Arial",15d)
  val t=new Text
  ...
  def update():Unit= {
    ...
    if(monthProperty.get()<=12&&monthProperty.get>=1) {
