...
class WorkingStageDialog(owner: Window) extends Dialog(owner)  {
  ...
  val statusEdit=new TextArea()
  statusEdit.preferredSize = new Dimension(100,100)
  statusEdit.peer.setFocusTraversalKeys(KeyboardFocusManager.FORWARD_TRAVERSAL_KEYS,null)
  statusEdit.peer.setFocusTraversalKeys(KeyboardFocusManager.BACKWARD_TRAVERSAL_KEYS,null)
  val dateEdit=new TextField()
  val okBut=new Button("OK")
  val cancelBut = new Button("Abbruch")
  var currentWorkingStage:Option[WorkingStage]=None
  ...
  contents=new BoxPanel(Orientation.Horizontal) {
    contents+=Swing.HStrut(20)+=new BoxPanel(Orientation.Vertical) {
      contents += new BoxPanel(Orientation.Horizontal) {
        ...
        descriptionEdit +=
        new BoxPanel(Orientation.Horizontal) {
        contents += new Label("Status:") += Swing.HStrut(40) += Swing.HGlue          
      } +=
        statusEdit+=
        new BoxPanel(Orientation.Horizontal) {
      ...
      listenTo(descriptionEdit.keys,statusEdit.keys)
      reactions += {
        case ev@KeyPressed(source, Key.Enter, modifiers, _) => if((modifiers& Key.Modifier.Control)>0) {
          ev.peer.consume()
          source match {
            case ed:TextArea=> ed.peer.insert("\n",ed.caret.position)
            case _ =>
          }
        }
        else if(modifiers==0) {
  ...
  def open(data:Option[WorkingStage]): Unit ={
    data match {
      case Some(d) =>
        nameEdit.text = d.name
        descriptionEdit.text = d.description
        statusEdit.text = d.status
        dateEdit.text = d.doneAt.toDateString match {
          case "0.0.0" => ""
          case o => o
        }
      case None =>
    ...
    currentWorkingStage=data
    visible = true
    nameEdit.requestFocus()
  }

  def store():Unit={
    val ref=currentWorkingStage match {
      case Some(work)=>work.ref
      case None => parentRef match {
        case Some(p)=>Reference(170, ClientQueryManager.createInstance(170, Array(OwnerReference(1, p))))
    ...
    ClientQueryManager.writeInstanceField(ref,2,StringConstant(statusEdit.text ))
    dateEdit.text match {
      case DateConstant(d)=>
          ClientQueryManager.writeInstanceField(ref,3,d)
      case ""=> if(currentWorkingStage.nonEmpty&& currentWorkingStage.get.doneAt.toDateString!="0.0.0")  ClientQueryManager.writeInstanceField(ref,2,NULL_DATE)
      case o=> JOptionPane.showInputDialog(this.peer,"Falsches Datumsformat:"+dateEdit.text );doClose=false
    }
    if(doClose) visible = false
  }
