...
support.isDataFlavorSupported(SpreadSheetTransferable.plainFlavor)
  ...
  private def addMissingCols(maxColNr:Int)= if(maxColNr>controller.tableModel.getColumnCount-2){
    ...
    if (!info.isDrop()) false else {
      ...
      if (flavors.contains(SpreadSheetTransferable.flavor)) {
        ...
        ClientQueryManager.executeAction(EMPTY_OWNERREF, List(controller.spreadSheetRef), "insertCells",
          Seq(("startCol", IntConstant(dropCol)), ("endCol", IntConstant(dropCol + data.numCols - 1)),
            ("startRow", IntConstant(dropRow)), ("endRow", IntConstant(dropRow + data.numRows - 1)), ("dx:", IntConstant(deltaX)), ("dy:", IntConstant(deltaY))) ++
            (data.cellValues map {
              case Some(CellTuple(exp, form)) => (exp.getTerm, EMPTY_EX)
              case None => ("", EMPTY_EX)
            }))
        ...
        info.getTransferable().getTransferData(InstanceSelection.flavor) match {
          case data: InstanceSelection => if (data.selection.nonEmpty) {
            ...
            ClientQueryManager.runInPool {
              ...
              yield (instData.fieldValue(modelIndex - 1).toString, EMPTY_EX)
              println("Lines:" + lines.mkString(" | "))
              ClientQueryManager.executeAction(EMPTY_OWNERREF, List(controller.spreadSheetRef), "insertCells", Seq(("startCol", IntConstant(dropCol)),
                ("endCol", IntConstant(dropCol + numCols - 1)), ("startRow", IntConstant(dropRow)), ("endRow", IntConstant(dropRow + data.selection.length - 1)),
                ("dx:", IntConstant(0)), ("dy:", IntConstant(0))) ++ lines)
            }
          }
        ...
        info.getTransferable().getTransferData(SpreadSheetTransferable.plainFlavor) match {
          case buffer: Array[Byte] =>
            ...
            val maxCols = lines.maxBy(_.size).size
            addMissingCols(maxCols + dropCol)
            //println("importiere CSV spalten:"+maxCols+" zeilen:"+lines.size)
            val fullData = lines.map(line => line.padTo(maxCols, "")).flatten.zip(Stream.continually(EMPTY_EX))
            ClientQueryManager.executeAction(EMPTY_OWNERREF, List(controller.spreadSheetRef), "insertCells", Seq(("startCol", IntConstant(dropCol)), ("endCol", IntConstant(dropCol + maxCols - 1)),
              ("startRow", IntConstant(dropRow)), ("endRow", IntConstant(dropRow + lines.length - 1)), ("dx:", IntConstant(0)), ("dy:", IntConstant(0))) ++ fullData)
          case other => println(other.getClass)
        }
        true
      } else false
...
class SpreadSheetTransferable(val ssRef:Reference,val startCol:Int,val startRow:Int,val numCols:Int,val numRows:Int,val cellValues:Array[Option[CellTuple]]) 
  ...
  override def toString= {val ret= (for(row<-0 until numRows) yield { 
   ...
   def toCSV= (for(row<-0 until numRows) yield { 
      (  for(col<-0 until numCols) yield cellValues(row*numCols+col) match {
...
object SpreadSheetTransferable {
  def apply(controller:SpreadSheetController)= {    
    ...
    val cellValues=for(row<-0 until numRows;col<-0 until numCols) 
      yield controller.tableModel.getCell(col+startCol,row+startRow) map(cellValue=>
        CellTuple(controller.replaceFieldReferences(cellValue.exp,true),controller.formatList.getCommonFormatForCell(col,row)))
