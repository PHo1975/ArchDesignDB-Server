...
class JavaClientSocket(val socket: Socket) extends Thread  with AbstractUserSocket {
  ...
  private def writeOut(st: String) = outStreamLock.synchronized {out.writeUTF(st); out.flush()}
	...
	private def wantWriteInstance(in:DataInputStream) = {
	  var error:CommandError=null
		val ref=Reference(in)
	...
	private def wantCreateInstances(in:DataInputStream) = {
		var error:CommandError=null
		val ownerCount=in.readInt
		val ownerArray:Array[OwnerReference]=(for (i <-0 until ownerCount) yield OwnerReference.read(in)).toArray
    val numInst=in.readInt
		...
		try {
			val ret=TransactionManager.doTransaction(userEntry.id,ClientCommands.createInstances.id.toShort,
			  ...
			  for(i <- 0 until numInst) {
			    val typ=in.readInt
			    val numFields=in.readByte
			    val fieldValues=for(i<-0 until numFields) yield Expression.read(in)
			    val inst= TransactionManager.tryCreateInstance(typ,ownerArray,true)
			    if (inst==null)	error=new CommandError("error creating typ "+typ,ClientCommands.createInstance.id,0)
			    else {
			      if(checkLinks) {
			        //System.out.println ("Create inst fieldV:"+fieldValues.mkString(",")+" ")
			        for(i<-0 until numFields;fv=fieldValues(i);if !fv.isNullConstant) {
			          //System.out.println("write :"+fv+" ix:"+i)
